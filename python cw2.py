...
class Stitcher:
    ...
    def stitch(self, img_left, img_right):
        ...
        # Already computed above for inliers
        # # homography already computed with inliers above

        result = self.warping(img_left, img_right, homography)

        # result = self.linear_blending(result)  # Disabled due to visual artifacts
    ...
    def draw_matches(self, img_left, img_right, kp_l, kp_r, matches):
        """
        Draw matching keypoints using OpenCV's drawMatches with match limiting and resized image safety.
        """
        if not matches or len(matches) < 4:
            print("Not enough matches to draw.")
            return

        # Resize both images to same height for consistent drawing
        ...
        for i, j in limited:
            try:
                match_obj.append(cv2.DMatch(_queryIdx=i, _trainIdx=j, _imgIdx=0, _distance=0))
            except:
                continue
    ...
    def find_homography(self, matches, kp_l, kp_r, threshold=5.0, iterations=None, return_inliers=False):
        ...
        if iterations is None:
            iterations = estimate_ransac_iterations(p=0.99, I=0.5, s=4)
        ...
        # Optional: refine homography using all inliers
        if best_H is not None and len(max_inliers) >= 4:
            ...
            refined_H = self.solve_homography(src_pts, dst_pts)
            best_H = refined_H
    ...
    def warping(self, img_left, img_right, H):
        ...
        # Transform right image corners to get final canvas size
        ...
        # Create a canvas and blend
        result = warped_right.copy()
        x_offset, y_offset = translation

        # Overlay left image using alpha blending
        for y in range(h1):
            for x in range(w1):
                px = img_left[y, x]
                ry = y + y_offset
...
if __name__ == "__main__":
    ...
    # Use OpenCV's high-level stitcher for best visual results
